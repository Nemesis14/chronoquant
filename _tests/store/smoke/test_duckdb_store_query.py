...
def _seed_store(data_dir: str) -> None:
    ...
    try:
        ...
        insert_feat_ohlcv_quant(conn, _feature_frame())
...
def test_ensure_tables_creates_core_schema(tmp_path: Path) -> None:
    """Verify ensure_tables creates ohlcv, target, and predictions tables."""
    ...
    assert {"ohlcv", "target", "predictions"}.issubset(tables)
...
def test_insert_helpers_are_append_only_and_sorted(tmp_path: Path) -> None:
    ...
    assert row_count(data_dir, "feat_ohlcv_quant") == 3
