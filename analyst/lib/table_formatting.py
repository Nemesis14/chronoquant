"""Table formatting helpers for ChronoQuant analysis notebooks.
...
def display_analysis_table(df: pd.DataFrame) -> None:
    ...
    display(_build_styler(df))


def analysis_table_html(df: pd.DataFrame) -> str:
    """Return styled HTML string for use in output:asis cells (print instead of display)."""
    return _build_styler(df).to_html()
