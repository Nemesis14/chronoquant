...
def plot_predictions_df(df: pd.DataFrame, ax=None):
	...
	if "spread" in df.columns:
		...
		ax2.axhspan(spread_min - 0.01, short_cutoff_val, alpha=0.2, color='red', label=f'SHORT Zone (≤ {short_cutoff_val:.6f})')
		ax2.axhspan(short_cutoff_val, long_cutoff_val, alpha=0.1, color='gold', label=f'NEUTRAL Zone')
		ax2.axhspan(long_cutoff_val, spread_max + 0.01, alpha=0.2, color='green', label=f'LONG Zone (≥ {long_cutoff_val:.6f})')
