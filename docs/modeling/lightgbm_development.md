...
sample/split rules live in `docs/modeling/sampling.md`; trigger and backtest
rules live in `docs/evaluation/strategy_evaluation.md`.
...
- confirm the data range and split rules using `docs/modeling/sampling.md`.

Quick check:

```bash
python -c "import json; d=json.load(open('samples/base_solusdt_fw60_futures_v1/folds.json')); print(len(d['folds']), 'folds, test:', d['test'])"
```

#### When to create a new sample_id

Create a new `sample_id` (do not reuse the old one) when any of these changed:

- data source: table name, asset, or spot/futures switch;
- target horizon or label definition;
- a data quality fix that materially changes the available range;
- holdout boundaries need to be reset.

Long and short models for the same asset and horizon must share the same `sample_id`.
...
print(len(d['folds']), 'folds')
print('test:', d['test'])
print('first fold train start:', d['folds'][0]['train_start'])
print('last fold train end:',   d['folds'][-1]['train_end'])
"
```
...
### 0c. Colab Preparation

The explore and refine search stages are compute-heavy and run on Google Colab.
Complete these local steps **before** opening the notebook:
...
git commit -m "add v4 model entries and updated sample"
git push
```
...
Update the **CONFIG** cell (model IDs, sample ID), then press **Ctrl+F9**.

**After Colab completes — copy artifacts back:**

```
F:\My Drive\chronoquant\models\<model_id>\  →  models\<model_id>\
```

Verify artifact presence before proceeding to step 2 (Feature Audit):
...
for mid in ['lgbm_solusdt_l_fw60_q90_local_v4', 'lgbm_solusdt_s_fw60_q10_local_v4']:
    ...
    for name in ['search_best.json', 'search_trials.jsonl', 'features_search.json']:
        fp = p / name
        print(mid, name, fp.stat().st_size if fp.exists() else 'MISSING')
"
```
...
folds = json.loads(Path("samples/base_solusdt_fw60_futures_v1/folds.json").read_text())["folds"]
fd = folds[-1]
...
Choose the final-fit data window according to `docs/modeling/sampling.md`.
