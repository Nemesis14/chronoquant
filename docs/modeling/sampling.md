...
## Target Column Value Semantics

Features table target columns (e.g. `trg_l_fw60_q90`, `trg_s_fw60_q10`) use
three distinct values:

| Value | Meaning |
|-------|---------|
| `1`   | Condition met — the forward window confirmed the event (e.g. price rose above threshold within the horizon) |
| `0`   | Condition not met — the forward window closed without the event occurring |
| `NULL` | Forward data not yet available — this row falls within the last `rolling_window` bars of the dataset at the time of computation |

### Why NULL matters

The target is computed on a reversed rolling window. For the last
`rolling_window` bars (e.g. 60 bars for `fw60`), the window does not contain
full forward data, so the ratio always equals `1.0`. Since the threshold is
typically `> 1.0`, these rows would receive `target = 0` instead of the correct
"unknown" state.

Rows inserted by incremental live sync are always in this edge zone at the time
of insertion. They are set to `NULL` rather than `0` to prevent them from
appearing as confirmed negatives during training.

### Handling NULL in the modeling pipeline

- `src/modeling/datasets.py` filters these rows with `dropna(subset=[target_col])`
  before building the training sample.
- Do not impute `NULL` targets as `0`. A `NULL` is genuinely unknown, not a
  confirmed negative.
- When auditing data freshness, a non-zero NULL rate in the most recent
  `rolling_window` rows is expected and correct. A NULL rate elsewhere indicates
