#!/usr/bin/env python3
# =============================================================================
# Legfrissebb 1-2-1-2 setup vizualizáció — 1m japángyertya chart
# =============================================================================
...
print("Detektálás...")
setups = detect_1212(df, threshold=THRESHOLD)
...
# -------------------------------------------------------------------------
# Pivot indexek visszakeresése
# -------------------------------------------------------------------------
p0_idx = df[df["open_time"] == pd.Timestamp(latest["p0_time"])].index[0]
p4_idx = df[df["open_time"] == pd.Timestamp(latest["p4_time"])].index[0]

all_pivots = detect_pivots(df, THRESHOLD)
...
if not p_points:
    print("WARN: fallback direkt keresés...")
    window_df = df.iloc[p0_idx:p4_idx + 1]
    p1_idx = window_df["high"].idxmax()
    p2_idx = window_df.loc[p1_idx:, "low"].idxmin()
    p3_idx = window_df.loc[p2_idx:, "high"].idxmax()
    p_points = {
        "P0": (p0_idx,  float(df.loc[p0_idx, "low"])),
        "P1": (p1_idx,  float(df.loc[p1_idx, "high"])),
        "P2": (p2_idx,  float(df.loc[p2_idx, "low"])),
        "P3": (p3_idx,  float(df.loc[p3_idx, "high"])),
        "P4": (p4_idx,  float(df.loc[p4_idx, "low"])),
    }
...
chart_end   = min(len(df) - 1, right_edge + CONTEXT_RIGHT)
chart_df    = df.iloc[chart_start: chart_end + 1].copy().reset_index(drop=True)
...
# Pivot pontok
# -------------------------------------------------------------------------
COLORS  = {"P0": "#ffeb3b", "P1": "#4fc3f7", "P2": "#ffeb3b", "P3": "#4fc3f7", "P4": "#ffeb3b"}
OFFSETS = {"P0": -0.6, "P2": -0.6, "P4": -0.6, "P1": +0.6, "P3": +0.6}
...
ax.plot(line_xs, line_ys, color="#b0bec5", linewidth=1.4, linestyle="--", zorder=4, alpha=0.8)

for label, (xi, price) in pvt_chart.items():
    ax.scatter(xi, price, s=90, color=COLORS[label], zorder=6, marker="o")
    ax.annotate(
        f"{label}\n{price:.2f}",
        xy=(xi, price), xytext=(xi, price + OFFSETS[label]),
        fontsize=8.5, color=COLORS[label], ha="center", va=VA[label],
        fontweight="bold", zorder=7,
    )

# -------------------------------------------------------------------------
# P4 confirmation zóna
# -------------------------------------------------------------------------
if p4_conf_bar is not None:
    ...
    ax.axvline(p4_low_xi, color="#ef5350", linewidth=1.2, linestyle=":", zorder=5, alpha=0.8)
    ax.text(p4_low_xi + 0.3, ax.get_ylim()[0], "P4 low", color="#ef5350", fontsize=7.5, va="bottom", alpha=0.9)

    ax.axvline(p4_conf_xi, color="#66bb6a", linewidth=1.6, linestyle="--", zorder=5, alpha=0.9)
    ax.text(p4_conf_xi + 0.3, ax.get_ylim()[0], "P4 conf.\n(signal)", color="#66bb6a", fontsize=7.5, va="bottom")

    p3_price = pvt_chart["P3"][1]
    ax.hlines(p3_price, p4_conf_xi, len(chart_df) - 1, colors="#ffa726", linewidths=1.2, linestyles="--", zorder=4, alpha=0.85)
    ax.text(len(chart_df) - 1, p3_price + 0.02, f"P3 trigger  {p3_price:.2f}",
            color="#ffa726", fontsize=7.5, ha="right", va="bottom")
...
info = (
    ...
    f"Score = {latest['score']:.3f}  |  Threshold = {THRESHOLD*100:.1f}%  |  TF = 1m"
)
ax.text(0.5, 0.97, info, transform=ax.transAxes, fontsize=9, color="#cfd8dc",
        ha="center", va="top",
...
ax.set_title(
    f"SOLUSDT 1m — Elliott 1-2-1-2 bullish setup  ({latest['conf_time']})",
    fontsize=13, color="white", pad=12,
)

plt.tight_layout()
out_path = "docs/analysis/eliot_waves/latest_1212_1m_setup.png"
