...
# Drift is flagged at half the max threshold (review) or full threshold (unstable/decayed).
...
def analyze_stability(
    ...
    # --- time range for bucket index formula ---
    min_ts_raw = conn.execute(
        "SELECT MIN(open_time)::VARCHAR FROM quant_train"
    ).fetchone()[0]  # type: ignore[index]
    min_ts = str(min_ts_raw)[:19]  # "YYYY-MM-DD HH:MM:SS"
    ...
    for col in feat_cols:
        ...
        # --- per-bucket Spearman ρ with each target ---
        ...
        # --- global (baseline) Spearman for drift reference ---
        ...
        # --- merge stats + spearman on bucket_idx ---
