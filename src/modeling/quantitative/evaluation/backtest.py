...
# =============================================================================
# Purpose:
#  - Generate model predictions over a historical interval
#  - Simulate reusable probability-threshold trading strategies
#  - Persist trade, equity, and summary artifacts
# =============================================================================
...
import utils
from database.store import duckdb_query
...
# =============================================================================
# Purpose:
#  - Load one strategy from config/strategies.json and run a backtest
# =============================================================================
def run_configured_strategy(strategy_id: str) -> dict:
    cfg = utils.load_strategies_config()
    strategies = cfg.get("strategies", {})
    if strategy_id not in strategies:
        raise ValueError(f"Strategy not found in config/strategies.json: {strategy_id}")
    return run_strategy_backtest(strategy_id, strategies[strategy_id])


# =============================================================================
# run_strategy_backtest(strategy_id: str, strategy_cfg: dict) -> dict
# =============================================================================
# Purpose:
#  - Build prediction/price frame
#  - Simulate strategy
...
def run_strategy_backtest(strategy_id: str, strategy_cfg: dict) -> dict:
    ...
    (output_dir / "summary.json").write_text(json.dumps(summary, indent=4), encoding="utf-8")
    (output_dir / "strategy_config.json").write_text(
        json.dumps(strategy_cfg, indent=4),
        encoding="utf-8",
    )
...
# =============================================================================
# simulate_long_probability_strategy(...) -> tuple[pd.DataFrame, pd.DataFrame, dict]
# =============================================================================
# Purpose:
#  - Run LONG/FLAT strategy with threshold entry, rearm, TP/SL, trailing, time exit
# =============================================================================
...
# =============================================================================
# summarize_trades(...) -> dict
# =============================================================================
# Purpose:
#  - Calculate reusable trade-level and equity-level backtest metrics
# =============================================================================
...
# =============================================================================
# write_backtest_report(...) -> None
# =============================================================================
# Purpose:
#  - Save a simple HTML report for one backtest result
# =============================================================================
def write_backtest_report(
    ...
    exit_df = pd.DataFrame(
        [{"exit_reason": key, "count": value} for key, value in summary.get("exit_reasons", {}).items()]
    )
...
# =============================================================================
# _profit_factor(wins: pd.Series, losses: pd.Series) -> float | None
# =============================================================================
# Purpose:
#  - Return sum of wins divided by absolute sum of losses
# =============================================================================
