...
def fetch_predictions_df(lookback_minutes: int = LOOKBACK_MINUTES, print_status: bool = True) -> pd.DataFrame:
	...
	with sqlite3.connect(db_path) as conn:
		select_cols = ", ".join(["open_time", "signal"] + pred_cols)
	...
	long_col = next((c for c in df.columns if "_l_" in c and c.endswith('_p')), None)
	short_col = next((c for c in df.columns if "_s_" in c and c.endswith('_p')), None)
	if long_col and short_col:
		df['spread'] = pd.to_numeric(df[long_col], errors="coerce") - pd.to_numeric(df[short_col], errors="coerce")
...
def plot_predictions_df(df: pd.DataFrame, ax=None):
	...
	x_numeric = range(len(df))  # For background zones (indices)
	...
	if long_col and long_col in df.columns:
		...
		ax1.plot(x, long_p, color='dodgerblue', linewidth=2.5, label=f'LONG ({long_col})', marker='o', markersize=5)
	...
	if short_col and short_col in df.columns:
		...
		ax1.plot(x, short_p, color='crimson', linewidth=2.5, label=f'SHORT ({short_col})', marker='s', markersize=5)
	...
	if "spread" in df.columns:
		...
		# Get cut-off thresholds from config if available, otherwise use typical values
		try:
			model_cfg = utils.load_models_config()
			trading_cfg = model_cfg.get("models", {}).get(long_col.replace("_p", ""), {}).get("trading_strategy", {})
			long_cutoff_val = trading_cfg.get("long_cutoff", 0.0139)
			short_cutoff_val = trading_cfg.get("short_cutoff", -0.0142)
		except:
			long_cutoff_val = 0.0139
			short_cutoff_val = -0.0142
		
		# Add zone background colors
		spread_min = spread_vals.min()
		spread_max = spread_vals.max()
		
		ax2.axhspan(spread_min - 0.01, short_cutoff_val, alpha=0.2, color='red', label=f'SHORT Zone (â‰¤ {short_cutoff_val:.6f})')
		ax2.axhspan(short_cutoff_val, long_cutoff_val, alpha=0.1, color='gold', label=f'NEUTRAL Zone')
		ax2.axhspan(long_cutoff_val, spread_max + 0.01, alpha=0.2, color='green', label=f'LONG Zone (â‰Ą {long_cutoff_val:.6f})')
		
		# Plot spread as line with markers
		ax2.plot(x, spread_vals, color='navy', linewidth=2.5, label='Spread (long_p - short_p)', 
				marker='D', markersize=6, alpha=0.8)
		
		# Cut-off threshold lines
		ax2.axhline(y=long_cutoff_val, color='green', linestyle='--', lw=2, alpha=0.7, label=f'LONG Cut-off')
		ax2.axhline(y=short_cutoff_val, color='red', linestyle='--', lw=2, alpha=0.7, label=f'SHORT Cut-off')
		ax2.axhline(y=0, color='black', linestyle='-', lw=0.5, alpha=0.3)
		
		ax2.set_title('Trading Zones: Spread Over Time (Red=SHORT, Yellow=NEUTRAL, Green=LONG)', 
