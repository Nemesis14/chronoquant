"""Binance Futures exchange client for live and dry-run order placement.

Wraps the python-binance Futures API with a dry-run mode that simulates
fills at mark price without placing real orders.
"""
...
class BinanceFuturesClient:
    ...
    def set_leverage(self) -> None:
        ...
        try:
            ...
            self._client.futures_change_leverage(  # type: ignore[union-attr]
                symbol=self.symbol, leverage=self.leverage
            )
            logger.info("Leverage set: %s x %d", self.symbol, self.leverage)
    ...
    def get_mark_price(self) -> float:
        ...
        try:
            assert self._client is not None
            result = self._client.futures_mark_price(symbol=self.symbol)  # type: ignore[union-attr]
    ...
    def _place_order(self, side: str, quantity: float, reduce_only: bool) -> dict:
        ...
        try:
            assert self._client is not None
            response = self._client.futures_create_order(**params)  # type: ignore[union-attr]
            logger.info("Order placed: %s", response.get("orderId"))
            return response
        except Exception as exc:
